//
//  NSFileCoordinator+SSAdditions.m
//  SSFoundation
//
...
#import "NSFileCoordinator+SSAdditions.h"

@implementation NSFileCoordinator (SSAdditions)
...
static id _getResourceValue(NSURL *url, NSString *resourceKey, BOOL expectedOK, NSError **outError)
{
    ...
    if (![url getResourceValue:&resourceValue forKey:resourceKey error:&resourceError]) {
        if (expectedOK)
            NSLog(@"Error getting the resource %@ for %@: %@", url, resourceKey, resourceError);
        if (outError)
...
static NSNumber *_URLsReferToSameResource(NSURL *URL1, NSURL *URL2, NSError **outError)
{
    ...
    if (!volumeIdentifier1) {
        if (NO)
            return @NO;
        NSLog(@"Error getting the resource %@ for %@: %@", URL1, NSURLVolumeIdentifierKey, error);
        if (outError)
            *outError = error;
        return nil;
    }
    ...
    if (!volumeIdentifier2) {
        if (NO)
            return @NO;
        NSLog(@"Error getting the resource %@ for %@: %@", URL2, NSURLVolumeIdentifierKey, error);
        if (outError)
            *outError = error;
        return nil;
    }
    
    if (![volumeIdentifier1 isEqual:volumeIdentifier2])
        return @NO;
    ...
    if (![resourceIdentifier1 isEqual:resourceIdentifier2])
        return @NO;
    return @YES;
}
...
static BOOL _ensureParentDirectory(NSURL *url, NSError **outError)
{
    ...
    if ([[NSFileManager defaultManager] createDirectoryAtURL:parentDirectoryURL withIntermediateDirectories:YES attributes:nil error:&error])
        return YES;
    NSLog(@"Error creating directory %@: %@", parentDirectoryURL, error);
    if (outError)
        *outError = error;
    return NO;
}
